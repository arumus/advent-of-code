...
val rucksacks =
  scala.io.Source.fromResource(s"advent22/day3.txt").getLines().toList
...
val round2 = rucksacks
  .foldLeft((1, List.empty[String], 0)) {
    case ((index, group, acc), x) if (group.nonEmpty && index % 3 == 0) =>
      (
        index + 1,
        List.empty,
        acc + priority(sharedChar((group :+ x).map(_.toCharArray.toSet)))
      )
