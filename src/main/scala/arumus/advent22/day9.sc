...
def scenicScore(row: Int, col: Int): Int = {
  val left = (col - 1 until 0 by -1)
    .takeWhile(c => grid(row)(c) < grid(row)(col)).size + 1
  val right = (col + 1 until totalColumns-1)
    .takeWhile(c => grid(row)(c) < grid(row)(col)).size + 1
  val up = (row - 1 until 0 by -1)
    .takeWhile(r => grid(r)(col) < grid(row)(col)).size + 1
  val down = (row + 1 until totalRows-1)
    .takeWhile(r => grid(r)(col) < grid(row)(col)).size + 1
  left * right * up * down
}
