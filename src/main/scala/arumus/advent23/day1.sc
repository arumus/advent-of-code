...
def transformWords(line: String) = {
  ...
  @tailrec
  def replaceAtStart(input: String,
                     words: List[(String, Int)] = numWordsWithIndex): String =
    words match {
      case Nil => input
...
//Exercise 1
val result1 = input.map(transformAsNumber).sum

//Exercise 2
val result2 = input.map(transformWords).map(transformAsNumber).sum
