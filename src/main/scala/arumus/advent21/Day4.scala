...
object Day4 {
  ...
  def columnWon(board: Board, row: Int = 0, col: Int = 0, won: Boolean = true): Boolean =
    if (col == board.grid.head.length) false
    else if (row == board.grid.length && won) won
    else if (row == board.grid.length) columnWon(board, col = col + 1)
  ...
  def rowWon(board: Board, row: Int = 0, col: Int = 0, won: Boolean = true): Boolean =
    if (row == board.grid.length) false
    else if (col == board.grid(row).length && won) true
    else if (col == board.grid(row).length) rowWon(board, row = row + 1)
  ...
  def playBingo1(boards: List[Board], drawNumbers: List[Int], drawPos: Int = 0): Int =
    if (drawPos == drawNumbers.length) -1
    else {
      val winningBoard = boards.find(b => columnWon(b) || rowWon(b))
      if (winningBoard.nonEmpty) calculateScore(winningBoard, drawNumbers(drawPos - 1))
      else
        playBingo1(
          boards.map(board =>
  ...
  def playBingo2(
    ...
    if (drawPos == drawNumbers.length) calculateScore(winningOrder.lastOption, drawNumbers(drawPos - 1))
    else {
      val winningBoards  = boards.filter(b => columnWon(b) || rowWon(b))
      val updatedWinners = (winningOrder ++ winningBoards).distinctBy(_.id)
      if (updatedWinners.size == boards.size) calculateScore(updatedWinners.lastOption, drawNumbers(drawPos - 1))
      else
        playBingo2(
          boards.map(board =>
            board.copy(grid = board.grid.map(_.map(b => b.copy(marked = b.marked || b.value == drawNumbers(drawPos)))))
